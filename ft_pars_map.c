...
/*   Updated: 2022/09/15 19:28:36 by snino            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_pars_line_color(t_game *game, char *line)
{
	while (line && *line)
	{
		...
		{
			ft_lstadd_back(&game->color_list, ft_lstnew(ft_strdup(line)));
...
static void	ft_pars_color(t_game *game, char *line)
{
	...
	else if (ft_strnstr(line, "F ", 2) || ft_strnstr(line, "C ", 2))
	{
		...
		ft_error_handler(game, "Error\nINVALID_COLOR_IDENTIFIER", leave);
}
