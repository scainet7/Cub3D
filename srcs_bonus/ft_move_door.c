...
/*   Updated: 2022/10/27 22:13:07 by snino            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	close_vert_door(t_game *game)
{
	...
	{
		if (game->player->p_x <= (double)x - 0.3 || \
			game->player->p_x >= (double)x + 1.3)
		{
			game->map[game->door->open_vert[0]] \
			[game->door->open_vert[1]] = '2';
...
int	open_vert_door(t_game *game)
{
	...
	if (game->player->dir_x > 0)
		y = (int)((game->player->p_x + game->player->dir_x * SPEED) \
		+ WALL_DISTANCE);
	else
		y = (int)((game->player->p_x + game->player->dir_x * SPEED) \
		- WALL_DISTANCE);
...
int	close_horizone_door(t_game *game)
{
	...
	{
		if (game->player->p_y <= (double)y - 0.3 || \
		game->player->p_y >= (double)y + 1.3)
		{
			game->map[game->door->open_hor[0]] \
			[game->door->open_hor[1]] = '2';
...
int	open_horizone_door(t_game *game)
{
	...
	if (game->player->dir_y > 0)
		y = (int)((game->player->p_y + game->player->dir_y * SPEED) \
		+ WALL_DISTANCE);
	else
		y = (int)((game->player->p_y + game->player->dir_y * SPEED) \
		- WALL_DISTANCE);
